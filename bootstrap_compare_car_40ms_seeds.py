#!/usr/bin/env python3
"""
bootstrap_compare_car_40ms_seeds.py — Compara el agente SNN campeón de la corrida
"car_ttfs_first_spike_40ms" (mismos hiperparámetros que la config ganadora original, solo ventana de simulación x2 (rank02 de esta corrida, no el de mayor fitPeak de sus 3 ranks)) contra el agente ANN (PPO nativo, rl-tools/C++) mediante
...
Modelo SNN: dentro de la corrida car_ttfs_first_spike_40ms (11 semillas, ver
eval_p3_weights_40ms_rank02/car_best.csv), el modelo específico (una de las 11
...
eval_p3_weights.py (promedio a través de 11 seeds de evaluación):
  run_key=car_ttfs_first_spike_40ms  rank=2  seed_idx=5  peso=2.0
  (n_neurons=52, n_connections=78)
...
Uso:
  python bootstrap_compare_car_40ms_seeds.py                       # N=100, 10000 resamples
  python bootstrap_compare_car_40ms_seeds.py --n 200 --resamples 20000
  python bootstrap_compare_car_40ms_seeds.py --out-dir mis_resultados/
"""
import io
import os
import subprocess
...
# ── Modelo SNN ganador (ver eval_p3_weights_40ms_rank02/car_best.csv) ─────────
...
def main() -> None:
    ap = build_arg_parser(__doc__, "bootstrap_car_40ms_seeds")
    args = ap.parse_args()
    ...
    run_comparison(rewards_snn, rewards_ann, args,
                  suptitle="Racing Car (ventana x2, mismos hiperparámetros, 11 semillas) — SNN vs ANN (PPO nativo), bootstrap no pareado",
                  plot_stem="car_40ms_seeds_snn_vs_ann", ann_label="ANN (PPO)")
