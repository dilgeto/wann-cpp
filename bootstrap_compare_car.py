#!/usr/bin/env python3
"""
bootstrap_compare_car.py — Compara el mejor agente SNN (WANN+SNN) de Racing
Car contra el agente ANN (PPO nativo, rl-tools/C++) mediante bootstrap NO
pareado.
...
Modelo SNN: dentro de la configuración ganadora de Racing Car (mayor
...
eval_p3_weights.py (promedio a través de 11 seeds de evaluación):
  run_key=car_ttfs_first_spike  rank=2  seed_idx=0  peso=8.0
...
Uso:
  python bootstrap_compare_car.py                       # N=100, 10000 resamples
  python bootstrap_compare_car.py --n 200 --resamples 20000
  python bootstrap_compare_car.py --out-dir mis_resultados/
"""
import io
import os
import subprocess
...
# ── Modelo SNN ganador (ver eval_p3_weights.py / car_best.csv) ───────────────
...
def main() -> None:
    ap = build_arg_parser(__doc__, "bootstrap_car")
    args = ap.parse_args()
    ...
    run_comparison(rewards_snn, rewards_ann, args,
                  ...
                  plot_stem="car_snn_vs_ann", ann_label="ANN (PPO)")
