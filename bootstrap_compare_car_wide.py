#!/usr/bin/env python3
"""
bootstrap_compare_car_wide.py — Compara el agente SNN campeón de la corrida
"car_ttfs_first_spike_wide" (rangos de hiperparámetros ampliados, ver
run_car_wide.sh / p/car_wide_space.json) contra el agente ANN (PPO nativo,
...
Modelo SNN: dentro de la corrida car_ttfs_first_spike_wide (ver
eval_p3_weights_wide/car_best.csv, generado con
`eval_p3_weights.py --task car --run-key car_ttfs_first_spike_wide
--out-dir eval_p3_weights_wide`), el modelo específico (una de las 11
...
eval_p3_weights.py (promedio a través de 11 seeds de evaluación):
  run_key=car_ttfs_first_spike_wide  rank=2  seed_idx=9  peso=8.0
  (n_neurons=21, n_connections=28)
...
Uso:
  python bootstrap_compare_car_wide.py                       # N=100, 10000 resamples
  python bootstrap_compare_car_wide.py --n 200 --resamples 20000
  python bootstrap_compare_car_wide.py --out-dir mis_resultados/
"""
import io
import os
import subprocess
...
# ── Modelo SNN ganador (ver eval_p3_weights_wide/car_best.csv) ───────────────
SNN_EXECUTABLE  = "./build/wann_eval_weights_car"
SNN_BASE_CONFIG = "p/car_snn.json"
SNN_RUN_KEY     = "car_ttfs_first_spike_wide"
SNN_RANK        = 2
SNN_SEED_IDX    = 9
...
def main() -> None:
    ap = build_arg_parser(__doc__, "bootstrap_car_wide")
    args = ap.parse_args()
    ...
    run_comparison(rewards_snn, rewards_ann, args,
                  suptitle="Racing Car (rango ampliado) — SNN vs ANN (PPO nativo), bootstrap no pareado",
                  plot_stem="car_wide_snn_vs_ann", ann_label="ANN (PPO)")
