#!/usr/bin/env python3
"""
Animación del Acrobot con la mejor red WANN+SNN.

Modo un episodio:
  python replay_acrobot.py --csv log/snn_acrobot_best_replay.csv

Modo evolución (secuencia de generaciones):
  python replay_acrobot.py --dir log/snn_acrobot_replay/
  python replay_acrobot.py --dir log/snn_acrobot_replay/ --save
"""
...
parser.add_argument("--csv",   default="log/snn_acrobot_best_replay.csv")
...
# ─────────────────────────────────────────────────────────────────────────────
# Cargar trayectorias
# Columnas: step,cos_th1,sin_th1,cos_th2,sin_th2,dth1,dth2,action,reward
# ─────────────────────────────────────────────────────────────────────────────
def load_csv(path):
...
# ─────────────────────────────────────────────────────────────────────────────
# Precomputar ángulos y reward acumulado
# ─────────────────────────────────────────────────────────────────────────────
def precompute(traj):
    th1 = np.arctan2(traj[:, 2], traj[:, 1])   # atan2(sin_th1, cos_th1)
    th2 = np.arctan2(traj[:, 4], traj[:, 3])   # atan2(sin_th2, cos_th2)
    cr  = np.cumsum(traj[:, 8])
    return th1, th2, cr

precomp = [precompute(t) for _, t in trajs]

all_cr   = np.concatenate([cr for _, _, cr in precomp])
...
# Figura
# ─────────────────────────────────────────────────────────────────────────────
L = 1.0   # longitud de cada eslabón

fig = plt.figure(figsize=(11, 5), facecolor="white")
title_obj = fig.suptitle("WANN+SNN — Acrobot", fontsize=13, fontweight="bold")

gs = fig.add_gridspec(2, 2, width_ratios=[1, 1], hspace=0.5, wspace=0.35,
                      left=0.07, right=0.97, top=0.88, bottom=0.10)

ax_acro   = fig.add_subplot(gs[:, 0])
ax_reward = fig.add_subplot(gs[0, 1])
ax_action = fig.add_subplot(gs[1, 1])

# ── Panel acrobot ─────────────────────────────────────────────────────────────
ax_acro.set_xlim(-2.2, 2.2)
ax_acro.set_ylim(-2.2, 2.2)
ax_acro.set_aspect("equal")
ax_acro.grid(True, alpha=0.25, linewidth=0.5)
ax_acro.set_xlabel("x (m)", fontsize=9)
ax_acro.set_ylabel("y (m)", fontsize=9)
# Línea de meta (tip debe estar por encima de y = 1.0)
ax_acro.axhline(1.0, color="green", linewidth=1.2, linestyle="--", alpha=0.6, label="Meta")
ax_acro.plot(0, 0, "ko", markersize=6, zorder=5)   # pivote fijo
ax_acro.legend(fontsize=8, loc="upper right")

link1_line, = ax_acro.plot([], [], "o-", color="steelblue",  linewidth=3, markersize=6)
link2_line, = ax_acro.plot([], [], "o-", color="darkorange", linewidth=3, markersize=6)
tip_dot,    = ax_acro.plot([], [], "r*", markersize=10, zorder=6)
step_text    = ax_acro.text(0.03, 0.97, "", transform=ax_acro.transAxes,
                             fontsize=8, va="top",
...
ax_action.set_xticklabels(["Torque"], fontsize=8)
ax_action.set_ylabel("N·m", fontsize=8)
...
def update(frame):
    ...
    if ti != cur_ti[0]:
        ...
        label = f"Gen {gen:04d}  ({ti+1}/{len(trajs)})" if len(trajs) > 1 else "Acrobot"
        title_obj.set_text(f"WANN+SNN — {label}")
        ax_acro.set_title(f"Trayectoria — {label}", fontsize=10)

    i = si
    # Posiciones de los eslabones
    x1 =  L * np.sin(th1[i]);        y1 = -L * np.cos(th1[i])
    ...
    return (link1_line, link2_line, tip_dot, step_text,
            reward_line, reward_dot, action_bar)
