#!/usr/bin/env python3
"""
bootstrap_compare_car_mutAct070_seeds.py — Compara el agente SNN campeón de la corrida
"car_ttfs_first_spike_mutAct070_seeds" (mutAct=0.70, resto igual a la config ganadora, 11 semillas) contra el agente ANN (PPO nativo, rl-tools/C++) mediante
...
Modelo SNN: dentro de la corrida car_ttfs_first_spike_mutAct070_seeds (11 semillas, ver
eval_p3_weights_mutAct070_seeds/car_best.csv), el modelo específico (una de las 11
...
eval_p3_weights.py (promedio a través de 11 seeds de evaluación):
  run_key=car_ttfs_first_spike_mutAct070_seeds  rank=0  seed_idx=1  peso=8.0
  (n_neurons=54, n_connections=79)
...
Uso:
  python bootstrap_compare_car_mutAct070_seeds.py                       # N=100, 10000 resamples
  python bootstrap_compare_car_mutAct070_seeds.py --n 200 --resamples 20000
  python bootstrap_compare_car_mutAct070_seeds.py --out-dir mis_resultados/
"""
import io
import os
import subprocess
...
SNN_WEIGHT_INDEX = 5     # índice de peso, el de mayor reward (revalidación fresca) entre los 11 modelos
...
def main() -> None:
    ap = build_arg_parser(__doc__, "bootstrap_car_mutAct070_seeds")
    args = ap.parse_args()
    ...
    run_comparison(rewards_snn, rewards_ann, args,
                  suptitle="Racing Car (mutAct=0.70, 11 semillas) — SNN vs ANN (PPO nativo), bootstrap no pareado",
                  plot_stem="car_mutAct070_seeds_snn_vs_ann", ann_label="ANN (PPO)")
