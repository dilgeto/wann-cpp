#!/usr/bin/env python3
"""
bootstrap_compare_car_combined_seeds.py — Compara el agente SNN campeón de la corrida
"car_ttfs_first_spike_combined_seeds" (addConn=0.05 + mutAct=0.70 + ventana de simulación x2, 11 semillas) contra el agente ANN (PPO nativo, rl-tools/C++) mediante
...
Modelo SNN: dentro de la corrida car_ttfs_first_spike_combined_seeds (11 semillas, ver
eval_p3_weights_combined_seeds/car_best.csv), el modelo específico (una de las 11
...
eval_p3_weights.py (promedio a través de 11 seeds de evaluación):
  run_key=car_ttfs_first_spike_combined_seeds  rank=0  seed_idx=8  peso=3.0
  (n_neurons=44, n_connections=53)
...
Uso:
  python bootstrap_compare_car_combined_seeds.py                       # N=100, 10000 resamples
  python bootstrap_compare_car_combined_seeds.py --n 200 --resamples 20000
  python bootstrap_compare_car_combined_seeds.py --out-dir mis_resultados/
"""
import io
import os
import subprocess
...
# ── Modelo SNN ganador (ver eval_p3_weights_combined_seeds/car_best.csv) ─────────
SNN_EXECUTABLE  = "./build/wann_eval_weights_car"
SNN_BASE_CONFIG = "p/car_snn.json"
SNN_RUN_KEY     = "car_ttfs_first_spike_combined_seeds"
SNN_RANK        = 0
SNN_SEED_IDX    = 8
...
SNN_WEIGHT_INDEX = 3     # índice de peso, el de mayor reward (revalidación fresca) entre los 11 modelos
SNN_WEIGHT_VALUE = 3.0   # valor real del peso compartido (weight_vals[SNN_WEIGHT_INDEX])
...
def main() -> None:
    ap = build_arg_parser(__doc__, "bootstrap_car_combined_seeds")
    args = ap.parse_args()
    ...
    run_comparison(rewards_snn, rewards_ann, args,
                  suptitle="Racing Car (addConn=0.05 + mutAct=0.70 + ventana x2, 11 semillas) — SNN vs ANN (PPO nativo), bootstrap no pareado",
                  plot_stem="car_combined_seeds_snn_vs_ann", ann_label="ANN (PPO)")
