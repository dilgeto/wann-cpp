// wann_disc_mc – WANN evolution + SNN simulator + Gymnasium MountainCar-v0 (discrete).
//
// Usage:
//   ./wann_disc_mc [-d disc_mc_snn.json] [-p overrides.json] [-o prefix] [-s seed] [-v]
...
int main(int argc, char* argv[]) {
    std::string defaultHyp = "p/disc_mc_snn.json";
    std::string overrideHyp;
    std::string outPrefix  = "snn_disc_mc";
    uint32_t    seed       = 42;
    bool        debugLog   = false;
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if      (arg == "-d" && i+1 < argc) { defaultHyp  = argv[++i]; }
            ...
            std::cerr << "Usage: wann_disc_mc [-d default.json] [-p overrides.json]"
                         " [-o prefix] [-s seed] [-v]\n";
            return 1;
        }
    }
    ...
    std::cout << "Task: SNN MountainCar Discrete (Gymnasium MountainCar-v0)"
