// wann_car – WANN evolution + SNN simulator + rl-tools CarTrack.
//
// Usage:
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if      (arg == "-d" && i+1 < argc) { defaultHyp  = argv[++i]; }
            ...
            std::cerr << "Usage: wann_car [-d default.json] [-p overrides.json]"
                         " [-o prefix] [-s seed] [-v]\n";
            return 1;
        }
    }
    ...
    std::cout << "Task: SNN Car (CarTrack)"
        ...
        if ((gen % REPLAY_INTERVAL == 0 || gen == hyp.maxGen - 1)
            ...
            try {
                task.exportTrajectory(pop[eliteIdx].wVec, pop[eliteIdx].aVec,
                                      wann::SnnCarTask::WEIGHT_VALS[bestWi],
