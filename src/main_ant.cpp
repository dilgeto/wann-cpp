// wann_ant – WANN evolution + SNN simulator + rl-tools Ant-v4 (MuJoCo).
//
// Usage:
//   ./wann_ant [-d ant_snn.json] [-p overrides.json] [-o prefix] [-s seed] [-v]
...
int main(int argc, char* argv[]) {
    std::string defaultHyp = "p/ant_snn.json";
    std::string overrideHyp;
    std::string outPrefix  = "snn_ant";
    uint32_t    seed       = 42;
    bool        debugLog   = false;
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if      (arg == "-d" && i+1 < argc) { defaultHyp  = argv[++i]; }
            ...
            std::cerr << "Usage: wann_ant [-d default.json] [-p overrides.json]"
                         " [-o prefix] [-s seed] [-v]\n";
            return 1;
        }
    }
