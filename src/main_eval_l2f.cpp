// wann_l2f_eval – carga una red entrenada y evalúa N episodios.
//
// Uso:
//   ./wann_l2f_eval
//   ./wann_l2f_eval -f log/snn_l2f_best.out -d p/l2f_snn.json -n 20 -s 0
//   ./wann_l2f_eval -w 1.5 -n 50
//   ./wann_l2f_eval -n 20 -S best          (guarda el mejor episodio)
//   ./wann_l2f_eval -n 20 -S 3             (guarda el episodio 3)
//   ./wann_l2f_eval -n 20 -S best -o mi_replay.csv
...
int main(int argc, char* argv[]) {
    std::string netFile    = "log/snn_l2f_best.out";
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if      (arg == "-f" && i+1 < argc) { netFile    = argv[++i]; }
            ...
            std::cerr << "Uso: wann_l2f_eval [-f red.out] [-d config.json]"
    ...
    int    chosenWiIdx  = 0;
    double chosenWeight = wann::SnnL2FTask::WEIGHT_VALS[0];
    ...
    if (weightArg == "best") {
        ...
        if (wif) {
            wif >> chosenWiIdx;
            chosenWeight = wann::SnnL2FTask::WEIGHT_VALS[chosenWiIdx];
            ...
            for (int i = 0; i < wann::SnnL2FTask::N_WEIGHTS; ++i)
                std::cout << wann::SnnL2FTask::WEIGHT_VALS[i]
                          << "→" << std::fixed << std::setprecision(2) << allRewards[i]
                          << (i+1 < wann::SnnL2FTask::N_WEIGHTS ? "  " : "\n");
        ...
        // find closest index
        double best_dist = std::abs(wann::SnnL2FTask::WEIGHT_VALS[0] - chosenWeight);
        for (int i = 1; i < wann::SnnL2FTask::N_WEIGHTS; ++i) {
            double d = std::abs(wann::SnnL2FTask::WEIGHT_VALS[i] - chosenWeight);
