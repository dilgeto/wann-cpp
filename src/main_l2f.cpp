// wann_l2f – WANN evolution + SNN simulator + rl-tools L2F (Crazyflie quadrotor).
//
// Usage:
//   ./wann_l2f [-d l2f_snn.json] [-p overrides.json] [-o prefix] [-s seed] [-v]
...
int main(int argc, char* argv[]) {
    std::string defaultHyp = "p/l2f_snn.json";
    std::string overrideHyp;
    std::string outPrefix  = "snn_l2f";
    uint32_t    seed       = 42;
    bool        debugLog   = false;
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if      (arg == "-d" && i+1 < argc) { defaultHyp  = argv[++i]; }
            ...
            std::cerr << "Usage: wann_l2f [-d default.json] [-p overrides.json]"
                         " [-o prefix] [-s seed] [-v]\n";
            return 1;
        }
    }
    ...
    std::cout << "Task: SNN L2F (Crazyflie quadrotor)"
