...
//   ./wann_acrobot_eval -w 1.5 -n 50
//   ./wann_acrobot_eval -n 20 -S best          (guarda el mejor episodio)
//   ./wann_acrobot_eval -n 20 -S 3             (guarda el episodio 3)
//   ./wann_acrobot_eval -n 20 -S best -o mi_replay.csv
//   ./wann_acrobot_eval -i 4                   (sobrescribe ann_nInput del config,
...
int main(int argc, char* argv[]) {
    ...
    int    chosenWiIdx  = 0;
    double chosenWeight = wann::SnnAcrobotTask::WEIGHT_VALS[0];
    ...
    if (weightArg == "best") {
        ...
        if (wif) {
            wif >> chosenWiIdx;
            chosenWeight = wann::SnnAcrobotTask::WEIGHT_VALS[chosenWiIdx];
        ...
        // find closest index
        double best_dist = std::abs(wann::SnnAcrobotTask::WEIGHT_VALS[0] - chosenWeight);
        for (int i = 1; i < wann::SnnAcrobotTask::N_WEIGHTS; ++i) {
            double d = std::abs(wann::SnnAcrobotTask::WEIGHT_VALS[i] - chosenWeight);
